	...
	const handleImageUpload = async (
		...
		try {
			...
			const response = await fetch(
				`${process.env.NEXT_PUBLIC_SOCKET_SERVER}/api/upload`,
				{
					method: "POST",
					body: formData,
				}
			...
			const url = process.env.NEXT_PUBLIC_SOCKET_SERVER + data.url;
			setPartido((prev: Partido) => {
				const partidoActualizado = {
					...prev,
									...
									<fieldset className="fieldset min-w-[20rem] max-w-[20rem]">
										...
										{partido.liga && (
											<div>
												...
												<p className="fieldset-label">
													Opcional (Resultado que aparecerá si es partido de
													liga)
												</p>
											</div>
										)}
